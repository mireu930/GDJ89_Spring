...
const deleteComments = document.getElementById("deleteComments");
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (commentId) {
    commentId.addEventListener("click", async function() {
      console.log("click");
...
async function getList(page) {
  ...
  fetch(`listComments?productNum=${num}&page=${page}`)
  .then(res => res.text())
  .then(res => {
...
async function addComments() {
  ...
  commentsListResult.addEventListener("click", function(e) {
    if(e.target.classList.contains("page-item")){
      let num = pageItem.getAttribute("data-comment-num");
      getList(num);
    }
  })
